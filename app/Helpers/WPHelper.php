...
//    }
    ...
    public static function get_site_title_by_url($url)
    {
        ...
        //Get Body Page
        $html = WPHelper::get_html_page($url);
        if ($html === false) {
            return false;
        }
        ...
        if (class_exists('DOMDocument')) {
            ...
            return ((new WPFormatting)->wp_strip_all_tags($title) == "" ? false : (new WPFormatting)->wp_strip_all_tags($title));
        }
    ...
    public static function get_html_page($url)
    {
        ...
        //sanitize Url
        $parse_url = (new WPHttp)->wp_parse_url($url);
        $urls[]    = (new WPFormatting)->esc_url_raw($url);

        //Check Protocol Url
        if (!array_key_exists('scheme', $parse_url)) {
            $urls      = array();
            $url_parse = (new WPHttp)->wp_parse_url($url);
            foreach (array('http://', 'https://') as $scheme) {
                $urls[] = preg_replace('/([^:])(\/{2,})/', '$1/', $scheme . (new WPFunctions)->path_join((isset($url_parse['host']) ? $url_parse['host'] : ''), (isset($url_parse['path']) ? $url_parse['path'] : '')));
            }
        }

        //Send Request for Get Page Html
        foreach ($urls as $page) {
            $response = (new WPHttp)->wp_remote_get($page, array(
                'timeout'    => 30,
                'user-agent' => "Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.71 Safari/537.36"
            ));
            if ((new WPHttp)->wp_remote_get($response)) {
                continue;
            }
            $data = (new WPHttp)->wp_remote_retrieve_body($response);
//            if (is_wp_error($data)) {
//                continue;
//            }
            return ((new WPFormatting)->wp_strip_all_tags($data) == "" ? false : $data);
        }
...
//    }
    ...
    {
        $first_day = WPHelper::get_date_install_plugin();
