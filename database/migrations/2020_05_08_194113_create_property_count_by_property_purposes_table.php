...
class CreatePropertyCountByPropertyPurposesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('property_count_by_property_purposes', function (Blueprint $table) {
            $table->id();
            ...
            $table->string('property_purpose', 255);
            $table->unsignedBigInteger('property_count')->default(0);
            $table->softDeletes();
            $table->timestamps();
        });

        DB::statement('ALTER TABLE `property_count_by_property_purposes`
    ...
    {
        Schema::dropIfExists('property_count_by_property_purposes');
    }
}
