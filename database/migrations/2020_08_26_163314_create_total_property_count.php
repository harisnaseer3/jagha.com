...
class CreateTotalPropertyCount extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('total_property_count', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('property_count')->default(0);
            $table->unsignedBigInteger('agency_count')->default(0);
            $table->unsignedBigInteger('city_count')->default(0);
            $table->softDeletes();
            $table->timestamps();
        });

        DB::statement('ALTER TABLE `total_property_count`
    ...
    {
        Schema::dropIfExists('total_property_count');
    }
}
