                ...
                <div class="card">
                    @foreach(['active', 'pending', 'expired', 'deleted', 'rejected','sold'] as $status)
                        ...
                        <div id="collapseListing{{ ucfirst($status) }}" class="collapse {{ $params['status'] === $status ? 'show' : '' }}" aria-labelledby="headingListing{{ ucfirst($status) }}"
                            ...
                            <?php $route_params = ['status' => $status, 'user' => \Illuminate\Support\Facades\Auth::user()->getAuthIdentifier(), 'sort' => 'id', 'order' => 'desc', 'page' => 10]; ?>
...
{{--                                    @endif--}}
    ...
    <ul class="list-group">
        ...
        <li class="list-group-item {{ in_array($current_route_name, ['properties.create', 'properties.edit']) ? 'active' : '' }}"><a
                ...
                @if ($current_route_name === 'properties.edit') Post Advertisement @else Post Advertisement @endif</a></li>
