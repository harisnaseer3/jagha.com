...
class CreateGetPropertiesByAgencyIdProcedure extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        DB::unprepared('
        DROP PROCEDURE IF EXISTS getPropertiesByAgencyId;
        CREATE PROCEDURE getPropertiesByAgencyId(
        IN last_id BIGINT,
        IN agency_id BIGINT,
...
WHERE
    `properties`.`status` = "active" AND `properties`.`deleted_at` IS NULL AND `properties`.`agency_id` = agency_id
ORDER BY
...
');
    ...
    {
//        Schema::dropIfExists('get_properties_by_agency_id_procedure');
        DB::unprepared('DROP PROCEDURE IF EXISTS getPropertiesByAgencyId;');
    }
}
