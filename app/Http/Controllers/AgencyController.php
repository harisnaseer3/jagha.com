...
//        dd($paginatedSearchResults[6]);
        ...
        $agency_data = (new Agency)->select('agencies.title', 'agencies.user_id', 'agencies.description', 'agencies.status',
...
//            $city = (new City)->select('id', 'name')->where('name', '=', str_replace('_', ' ', $request->input('city')))->first();
    ...
    public function FeaturedAgencies()
    {
        return $this->_listingFrontend()
            ->where('agencies.featured_listing', '=', 1)->orderBy('agencies.featured_listing', 'desc')->limit(10)->get();
    }
